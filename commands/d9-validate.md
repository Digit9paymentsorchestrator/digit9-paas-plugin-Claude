...
- [ ] BLOCKER — `agent_transaction_ref_number` sent in the canonical create body; use `sender.agent_customer_number` as the partner idempotency key
...
- [ ] WARNING — Not storing the system `transaction_ref_number` alongside the partner's `agent_customer_number`
